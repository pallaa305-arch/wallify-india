#!/usr/bin/env python3
"""Daily wallpaper generator: adds 10 new REAL AI wallpapers (1 per category
rotating across all categories), updates the live manifest, commits & pushes to
GitHub so Render auto-deploys.

Keeps the EXISTING real-image site format:
  - files are JPGs in public/assets/
  - manifest entries: {"file", "category", "w", "h"}
Filenames continue from the current max index per category (no collisions).
"""
...
# Real-image categories + prompt templates (matches gen_real.py)
CATEGORIES = {
    "superbikes": [
...
def main():
    ...
    # current max index per category -> continue numbering
    maxidx = {}
    for x in data:
        c = x.get("category")
        f = x.get("file", "")
    ...
    cats = list(CATEGORIES.keys())  # 10 categories -> 10 wallpapers, 1 each
    n = 10
    added = []
    for i in range(n):
        cat = cats[i % len(cats)]
        ...
        if not success:
            print(f"  {cat} FAIL ({info}) - retry once")
            time.sleep(10)
            success, info = gen_one(cat, cur, tmpl)
        if success:
            maxidx[cat] = cur + 1
            ...
            print(f"  {cat} retry FAIL - skip")
        time.sleep(3)

    json.dump(data, open(man, "w"), indent=2)
    print(f"Added {len(added)} wallpapers. Total now: {len(data)}")
